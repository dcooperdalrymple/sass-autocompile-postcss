...
export default {
    ...
    maybeProcess(editor) {
        ...
        if (!this.sassAutocompile.isProcessing || !editor || !(editor.getURI) || !this.sassAutocompile.isSassFile(editor.getURI())) return false;

        console.log('Sass Autocompile PostCSS: Waiting for sass-autocompile to finish compilation');

        this.timestamp = new Date().getTime();
        var duration = 0;
        while (this.sassAutocompile.isProcessing && duration < SassAutocompilePostcssOptions.get('sassTimeout')) {
            duration = new Date().getTime() - this.timestamp;
        }

        if (duration > SassAutocompilePostcssOptions.get('sassTimeout')) {
            console.log('Sass Autocompile PostCSS: Timed out while waiting for sass-autocompile.');
        } else {
            var files = this.getOutputFiles(editor.getURI());
