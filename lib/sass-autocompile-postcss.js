...
export default {
    ...
    registerCommands() {
        ...
        this.subscriptions.add(atom.commands.add('atom-workspace', {
            ...
            'sass-autocompile-postcss:enable': () => this.enable(),
            'sass-autocompile-postcss:disable': () => this.disable(),
        }));
    },
    toggle() {
        if (!SassAutocompilePostcssOptions.get('enabled')) {
            this.enable();
        } else {
            this.disable();
        }
    ...
    enable(message = true) {
        SassAutocompilePostcssOptions.set('enabled', true);
        if (!!message && !!atom.notifications) {
            atom.notifications.addInfo('Sass Autocompile PostCSS <strong>enabled</strong>.');
        }
    },
    disable(message = true) {
        SassAutocompilePostcssOptions.set('enabled', false);
        if (!!message && !!atom.notifications) {
            atom.notifications.addInfo('Sass Autocompile PostCSS <strong>disabled</strong>.');
        }
    },
    ...
    maybeProcess(editor) {
        ...
        if (!SassAutocompilePostcssOptions.get('enabled')) return false;
    ...
    _maybeProcess(filename) {
        if (this.sassAutocompile.isProcessing && (new Date().getTime() - this.timestamp) < SassAutocompilePostcssOptions.get('sassTimeout')) {
            setTimeout(() => this._maybeProcess(filename), SassAutocompilePostcssOptions.get('processInterval'));
            return false;
        }
        ...
        if (SassAutocompilePostcssOptions.get('processDelay') > 0) {
            console.log('Sass Autocompile PostCSS: Waiting ' + (SassAutocompilePostcssOptions.get('processDelay') / 1000) + 's before processing compiled CSS files.');
            setTimeout(() => this.processFiles(files), SassAutocompilePostcssOptions.get('processDelay'));
        } else {
    ...
    prepareExecParameters(filename) {
        parameters = [
            filename,
            '--replace',
            '--config ' + atom.packages.getActivePackage('sass-autocompile-postcss').path + '/lib/postcss.config.js'
        ];
